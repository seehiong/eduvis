...
def _render_fact_boxes(spec, zx, zy, zw, zh, posting_group="G1", tracker=None) -> tuple[list[str], int]:
    ...
    # Pick a single font size for ALL items so none stands out as oversized.
    # Use large only when every item fits on one line at that size.
    all_texts = [item.get("text", "") for item in items]
    if all(len(_wrap(t, text_w, size_lg)) == 1 for t in all_texts):
        size, lead = size_lg, lead_lg
    else:
        size, lead = size_sm, lead_sm
